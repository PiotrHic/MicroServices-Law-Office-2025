...
import { Box, Container, Paper, Typography, Button, Stack, TextField, Alert, CircularProgress, Card, CardContent, Divider } from "@mui/material";
...
export default function UpdateLawyer() {
    ...
    const [updatedLawyer, setUpdatedLawyer] = useState(null);
    ...
    // ✅ Update po ID (nie zmieniamy, działa)
    const handleUpdateById = async () => {
        ...
        try {
            const res = await axios.put(
                `/api/lawyer/update/byId/${encodeURIComponent(idInput)}`,
                buildPayload(),
                { headers: { "Content-Type": "application/json" } }
            );
            setUpdatedLawyer(res.data);
            setSuccess("Lawyer updated successfully by ID!");
        } catch (err) {
            console.error(err);
            if (err.response) {
    ...
    const handleUpdateByName = async () => {
        ...
        try {
            ...
            const res = await axios.put(
                "/api/lawyer/update/byName",
                payload,
                {
                    params: { lawyerName: nameSearchInput.trim() }, // <-- query param
                    headers: { "Content-Type": "application/json" }
                }
            );

            setUpdatedLawyer(res.data);
            setSuccess("Lawyer updated successfully by Name!");
        } catch (err) {
            console.error(err);
            if (err.response) {
                if (err.response.status === 403) setError("Access denied (403).");
                    ...
                    <Stack spacing={3}>
                        ...
                        {updatedLawyer && (
                            <Card sx={{ mt: 2, p: 2, background: "#d1c4e9" }}>
                                <CardContent>
                                    <Typography variant="h6" sx={{ fontWeight: "bold" }}>Updated Lawyer:</Typography>
                                    <Divider sx={{ my: 1 }} />
                                    <Typography><strong>ID:</strong> {updatedLawyer.id}</Typography>
                                    <Typography><strong>Name:</strong> {updatedLawyer.name}</Typography>
                                    <Typography>
                                        <strong>Cases:</strong>
                                        {updatedLawyer.lawCaseList?.length
                                            ? updatedLawyer.lawCaseList.map(c => c.id).join(", ")
                                            : "No cases"}
                                    </Typography>
                                </CardContent>
                            </Card>
